...
class MiniMaxPlayer:
    ...
    def move(self, game, time_left):
        ...
        if not game.get_player_moves(self):
            ...
            best_move, score = minimax(self, game, time_left, depth=self.search_depth)
            return best_move
            # score_max = float("-inf")
            # try:
...
class AlphaBetaPlayer:
    ...
    def __init__(self, search_depth=3, eval_fn=CustomEvalFn(), name="AlphaBetaPlayer"):
        """Initializes your player.
    ...
    def move(self, game, time_left):
        ...
        if not game.get_player_moves(self):
            ...
            # except TimeoutError:
            #     print("AlphaBeta Reached Depth: {}".format(depth))
            #     return best_move
...
########################################################################################################################
# So we are trying to predict all the possible states from a given state to a max depth level
# Then for each state, we are trynig to get some sort of an evalation score to know how good the state is
# Ideally we want to get rid of all the states which are useless, i.e. which have a very less chance of winning
########################################################################################################################
...
def minimax(player, game, time_left, depth, my_turn=True, current_depth=0):
    ...
    if current_depth == depth:
        return player.utility(game, player)
...
def alphabeta(player, game, time_left, depth, alpha=float("-inf"), beta=float("inf"), my_turn=True, current_depth=0):
    ...
    if my_turn:
        ...
        for available_move in active_moves:
            ...
            # Every time ret_val is computed, we append it to the score_list and sort it again
            # We then return the max value of all the scores back to the user for a given depth
            best_move, max_score = sorted(score_list, key=lambda x: x[1])[-1]
            if max_score >= beta:
                break
        ...
        for available_move in active_moves:
            ...
            if type(ret_val) == tuple:
                ret_val = ret_val[1]
            score_list.append((available_move, ret_val))
            best_move, min_score = sorted(score_list, key=lambda x: x[1])[0]
            if min_score <= alpha:
                break
